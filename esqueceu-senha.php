...
if (isset($_POST['ok'])) {
    ...
    // Consulta para verificar o e-mail no banco
    $sql = "SELECT senha, id_usuario FROM usuario WHERE email_usuario = '$email'";
    ...
    if (empty($erro) && $total > 0) {
        $novasenha = substr(md5(time()), 0, 6);
        $nvscriptografada = md5(md5($novasenha));

        if (mail($email, "Nova Senha", "Sua nova senha: " . $novasenha)) {
            $sql = "UPDATE usuario SET senha = '$nvscriptografada' WHERE email_usuario = '$email'";
            $res = $conn->query($sql) or die($conn->error);
        }
    }
}
        ...
        <div class="card">
            <div class="card2">
                ...
                <form method="POST" action="">
                    <p id="heading">Digite seu E-mail</p>
                    <div class="field">
